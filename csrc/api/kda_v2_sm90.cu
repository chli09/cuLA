...
#include "kda/sm90/kda_fwd_v2_wmma.cuh"
#include "kda/sm90/kda_fwd_v2_wsp.cuh"
...
kda_fwd_v2(
    ...
    if (backend == 0) {
        ...
        // WMMA: Tensor-Core-based K2
        TORCH_CHECK(chunk_offsets.dtype() == torch::kInt32, "chunk_offsets must be int32");
        TORCH_CHECK(chunk_size == 64, "wmma backend requires chunk_size=64");
        ...
        // FUSED: step 1+5 share state b_frag loads (Step H-1)
        TORCH_CHECK(chunk_offsets.dtype() == torch::kInt32, "chunk_offsets must be int32");
        TORCH_CHECK(chunk_size == 64, "fused backend requires chunk_size=64");

        const float* init_ptr = initial_state_.has_value()
            ? initial_state_.value().data_ptr<float>()
            : nullptr;

        kda::sm90::v2::fused_impl::launch_kda_fwd_v2_fused(
            reinterpret_cast<const __nv_bfloat16*>(ws_qd.data_ptr()),
            reinterpret_cast<const __nv_bfloat16*>(ws_kd.data_ptr()),
            reinterpret_cast<const __nv_bfloat16*>(ws_kr.data_ptr()),
            ws_gt.data_ptr<float>(),
        ...
        // PART: warp-partition K2 (parallel matmul pairs)
        TORCH_CHECK(chunk_offsets.dtype() == torch::kInt32, "chunk_offsets must be int32");
        TORCH_CHECK(chunk_size == 64, "part backend requires chunk_size=64");

        const float* init_ptr = initial_state_.has_value()
            ? initial_state_.value().data_ptr<float>()
            : nullptr;

        kda::sm90::v2::part_impl::launch_kda_fwd_v2_part(
            reinterpret_cast<const __nv_bfloat16*>(ws_qd.data_ptr()),
            reinterpret_cast<const __nv_bfloat16*>(ws_kd.data_ptr()),
            reinterpret_cast<const __nv_bfloat16*>(ws_kr.data_ptr()),
            ws_gt.data_ptr<float>(),
        ...
        // WSP2: 2-stage warp-specialized K2 (cp.async pipeline)
        TORCH_CHECK(chunk_offsets.dtype() == torch::kInt32, "chunk_offsets must be int32");
        TORCH_CHECK(chunk_size == 64, "wsp2 backend requires chunk_size=64");

        const float* init_ptr = initial_state_.has_value()
            ? initial_state_.value().data_ptr<float>()
            : nullptr;

        kda::sm90::v2::wsp2_impl::launch_kda_fwd_v2_wsp2(
            reinterpret_cast<const __nv_bfloat16*>(ws_qd.data_ptr()),
            reinterpret_cast<const __nv_bfloat16*>(ws_kd.data_ptr()),
            reinterpret_cast<const __nv_bfloat16*>(ws_kr.data_ptr()),
            ws_gt.data_ptr<float>(),
        ...
        // WSP: warp-specialized C++ K2 (cp.async loads + WMMA math team)
        TORCH_CHECK(chunk_offsets.dtype() == torch::kInt32, "chunk_offsets must be int32");
        TORCH_CHECK(chunk_size == 64, "wsp backend requires chunk_size=64");

        const float* init_ptr = initial_state_.has_value()
            ? initial_state_.value().data_ptr<float>()
            : nullptr;

        kda::sm90::v2::wsp_impl::launch_kda_fwd_v2_wsp(
            reinterpret_cast<const __nv_bfloat16*>(ws_qd.data_ptr()),
            reinterpret_cast<const __nv_bfloat16*>(ws_kd.data_ptr()),
            reinterpret_cast<const __nv_bfloat16*>(ws_kr.data_ptr()),
            ws_gt.data_ptr<float>(),
