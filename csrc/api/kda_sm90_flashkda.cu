...
//
// Adapted from FlashKDA (csrc/flash_kda.cpp). The dispatch macros and TORCH_CHECKs
// follow the upstream FlashKDA layout; the entry point is renamed to
// `flashkda_fwd_prefill` and registered through cuLA's central pybind module.

#include <ATen/cuda/CUDAContext.h>
#include <c10/cuda/CUDAStream.h>
#include <torch/extension.h>

#include "kda/sm90_flashkda/fwd.h"

namespace {

constexpr int kFlashKdaChunk = 16;
constexpr int kFlashKdaHeadDim = 128;

}  // namespace

int64_t
flashkda_get_workspace_size(int64_t T_total, int64_t H, int64_t N) {
    // Upper bound: each of N sequences adds at most 1 extra tile vs floor division.
    int64_t total_tiles = (T_total + kFlashKdaChunk - 1) / kFlashKdaChunk + N;

    static_assert(kFlashKdaChunk * kFlashKdaHeadDim * 2 % 128 == 0,
                  "k_decayed/q_decayed/k_restored size must be 128-byte aligned");
    static_assert(kFlashKdaHeadDim * 4 % 128 == 0, "g_total size must be 128-byte aligned");
    static_assert(kFlashKdaChunk * kFlashKdaChunk * 2 % 128 == 0, "INV/Mqk size must be 128-byte aligned");

    int64_t per_tile_bytes = 3 * kFlashKdaChunk * kFlashKdaHeadDim * 2  // k_decayed + q_decayed + k_restored (bf16)
                             + kFlashKdaHeadDim * 4                     // g_total (fp32)
                             + 2 * kFlashKdaChunk * kFlashKdaChunk * 2; // INV + Mqk (bf16)
...
flashkda_fwd_prefill(
    ...
    TORCH_CHECK(
        q.is_cuda() && k.is_cuda() && v.is_cuda() && g.is_cuda() && beta.is_cuda() && out.is_cuda() &&
            workspace.is_cuda(),
        "all tensors must be on CUDA");
    TORCH_CHECK(
        q.is_contiguous() && k.is_contiguous() && v.is_contiguous() && g.is_contiguous() && beta.is_contiguous() &&
    ...
    if (has_state_in) {
        ...
        TORCH_CHECK(
            is.dtype() == torch::kBFloat16 || is.dtype() == torch::kFloat32, "initial_state must be bfloat16 or float32");
    ...
    if (has_state_out) {
        ...
        TORCH_CHECK(
            fs.dtype() == torch::kBFloat16 || fs.dtype() == torch::kFloat32, "final_state must be bfloat16 or float32");
        if (fs.dtype() == torch::kFloat32) state_fp32 = true;
    }
    if (has_state_in && has_state_out) {
        TORCH_CHECK(
            initial_state->dtype() == final_state->dtype(), "initial_state and final_state must have the same dtype");
    }
    ...
    TORCH_CHECK(beta.size(0) == B && beta.size(1) == T_seq && beta.size(2) == H, "beta must be [B, T, H] matching q");
    ...
    TORCH_CHECK(D == kFlashKdaHeadDim, "currently only supports D == 128");
    ...
    // K1 expects beta in [H, T_total] (1D TMA, no T alignment constraint).
    ...
    if (is_varlen) {
        ...
        TORCH_CHECK(cu.is_cuda(), "cu_seqlens must be on CUDA");
        TORCH_CHECK(cu.dtype() == torch::kLong, "cu_seqlens must be int64");
        TORCH_CHECK(cu.dim() == 1, "cu_seqlens must be 1D");
        N_val = cu.numel() - 1;
        TORCH_CHECK(N_val > 0, "cu_seqlens must have at least 2 elements");
        cu_seqlens_dev = cu.data_ptr<int64_t>();
    } else {
        N_val = B;
    }
    ...
    if (has_state_in) {
        ...
        TORCH_CHECK(
            is.size(0) == N_val && is.size(1) == H && is.size(2) == D && is.size(3) == D,
    ...
    if (has_state_out) {
        ...
        TORCH_CHECK(
            fs.size(0) == N_val && fs.size(1) == H && fs.size(2) == D && fs.size(3) == D,
            "final_state must be [N, H, D, D]");
    }

    int total_tiles;
    if (is_varlen) {
        total_tiles = int((T_total + kFlashKdaChunk - 1) / kFlashKdaChunk + N_val);
    } else {
        total_tiles = int(N_val * ((T_seq + kFlashKdaChunk - 1) / kFlashKdaChunk));
    }

#define LAUNCH(HI, HO, FP32, VL)                                                                                       \
    launch_fwd<kFlashKdaHeadDim, HI, HO, FP32, VL>(                                                                    \
        q_ptr,                                                                                                         \
        k_ptr,                                                                                                         \
        v_ptr,                                                                                                         \
        g_ptr,                                                                                                         \
